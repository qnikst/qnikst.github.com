...
one Haskell application. Then explain how we can assign start threaded
RTS, so it is aware of the CPU layout on your system. If you want to
skip the story, you can proceed to the solution directly.
...
We wanted to test the capabilities of the application and RPS that
application can support. We used Yandex Tank for this purpose.
[Yandex tank][tank] can generate a load on the site and build
[fantastic reports][report-0] (may require GitHub authorization).

Results were quite interesting. At first, everything went well, and
...
gave a pressure about `2k` RPS, the situation became worse.
Application stopped being responsive. The application was able to
...
life. Keeping that in mind GC was tunned beforehand, and I prepared
some metrics. However, this time situation was quite OK, garbage
collector pauses were all <10ms, and 99% of the total time program
spent on actual work and not garbage collection. Memory usage was too
big ~700Mb, and I didn't expect that.
...
ratio of time spent in mutator to the total running time.

Other parts of the system were not under stress and were capable of
the handling higher load. So the issue was with my program.
...
that is comparable to the one where stress tests were running, and that
...
After that single change situation drastically changed, the program was
...
Connect time is still terrible here, but it was fixed by reconfiguring
and improving network, and is out of the scope.
...
as you have cores. However, `RTS` makes no difference between real
and virtual cores. It appears that `RTS` could not make enough
benefit from the virtual cores, and performance is not good enough
in that case.

While it is pretty impressive, that we can optimise the program
without any changes in source code, it's interesting, what can we
do in the general case. It's problematic that we can not set good
options that work on any CPU and have decent performance. To find
an answer, I've started the project [ht-no-more]. It lives in my
playground for now, but I can extract it to the separate repo. I
hope that at some point it will be mature enough to be used as a
library or even land to the RTS.

## Solution <a name="solution"></a>

The idea is to gather information about the architecture during
the program startup and then set up the RTS with proper
configuration. We want:

1. collect information about the CPU (how many real and how many virtual cores do we have);
2. do not allow RTS to run work on the virtual cores;
...
The first question is how can we tie our process to the `CPU`.
There are [sched_getaffinity()][sched-affinity] and
[sched_setaffinity()][sched-affinity] calls. Those methods
perform hard wiring of the process and all it's descendent
processes to the given CPUs. So we can use them to mask CPU
...
First, we let's write a simple `c` program that explains the API.
...
option that tells GHC not to create it's own "main" and use the "main"
...
function that tells how many capabilities RTS created.
...
and run with `RTS +N` you'll see expected logs, but the program report
...
configured processors and creates that number of capabilities. Instead,
...
So get the desired result. We change cabal to provide required
...
There is some further work.

1. the code could be cleared and prettified;
...
if you run a program with `-xm` flag (that pins capability to CPU) may fail.
...
and reuse in the other projects.
